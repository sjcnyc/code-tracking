...
#>
	...
	foreach ($Server in $ComputerName)
	{
		...
		$SourceDrives = Invoke-Command -Session $Session -ScriptBlock {
			Get-PSDrive | Where-Object { $_.Free }
		}
		$SourceDrives = $SourceDrives | Select-Object @{ N = 'Drive'; E = { $_.Root } },
											   Used,
		...
		log ($SourceDrives | Select-Object 'Drive', 'Used(GB)', 'Free(GB)', 'Total(GB)', 'Free(%)' | Format-Table -Auto | Out-String).Trim() -LogFile $LogFile
		
		$FileShares = Invoke-Command -Session $Session -ScriptBlock {
			Get-WmiObject -Class Win32_Share
		} | Select-Object Name, Path, Description
		$FileShares = $FileShares |
		Where-Object { -not $_.Description.StartsWith('Default') -and $_.Path } |
		Sort-Object Path # Not need default shares
		log 'File Shares information on source server', $Server Green, Cyan $LogFile
		log ($FileShares | Format-Table -Auto | Out-String).Trim() -LogFile $LogFile
		...
		} | Select-Object ShareName, UserName, ComputerName, ActiveTime # in seconds
		if ($ShowConnectedUsers) { log ($ConnectedUsers | Sort-Object ShareName | Format-Table -AutoSize | Out-String) -LogFile $LogFile }
		$Groups = $ConnectedUsers | Group-Object -Property ShareName | Sort-Object Count -Descending | Select-Object Name, Count
		log ($Groups | Format-Table -AutoSize | Out-String).Trim() -LogFile $LogFile
		...
		log ($SharePerms | Sort-Object ShareName | Format-Table -AutoSize | Out-String).Trim() -LogFile $LogFile
		...
		$FileShares | % {
			...
			$Props = [ordered]@{
				...
				ConnectedUsers = $(($Groups | Where-Object { $_.Name -eq $ShareName }).Count)
				'DriveTotal(GB)' = $(($SourceDrives | Where-Object { $_.Drive[0] -eq $DriveLetter }).'Total(GB)')
				'DriveUsed(GB)' = $(($SourceDrives | Where-Object { $_.Drive[0] -eq $DriveLetter }).'Used(GB)')
				'DriveFree(GB)' = $(($SourceDrives | Where-Object { $_.Drive[0] -eq $DriveLetter }).'Free(GB)')
				'DriveFree(%)' = $(($SourceDrives | Where-Object { $_.Drive[0] -eq $DriveLetter }).'Free(%)')
				ShareUser = $(($SharePerms | Where-Object { $_.ShareName -eq $ShareName }).SecurityPrincipal)
				SharePerms = $(($SharePerms | Where-Object { $_.ShareName -eq $ShareName }).FileSystemRights)
				ShareAccess = $(($SharePerms | Where-Object { $_.ShareName -eq $ShareName }).AccessType)
			}
			$ShareInfo += New-Object -TypeName PSObject -Property $Props
		}
		log ($ShareInfo | Sort-Object ConnectedUsers -Descending | Format-Table -AutoSize | Out-String) -LogFile $LogFile
