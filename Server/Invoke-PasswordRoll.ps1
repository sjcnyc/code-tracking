function Invoke-PasswordRoll
{
  ...
  $RemoteRollScript = {
    ...
    $LocalUsers = Get-WmiObject -Class Win32_UserAccount -Filter 'LocalAccount=true' | ForEach-Object -Process {
      $_.Name
    }
    ...
    foreach ($User in $LocalUsers)
    {
      ...
      {
        Write-Warning -Message "Server: '$($TargettedServerName)' has a local account '$($User)' whos password is NOT being changed by this script"
      }
    }
    ...
    foreach ($LocalAdmin in $LocalAccounts)
    {
      ...
      if ($LocalUsers -icontains $LocalAdmin)
      {
        ...
        {
          Write-Error -Message "Error changing password for user:$($LocalAdmin) on server:$($TargettedServerName)"
        }
      }
  ...
  if ($PsCmdlet.ParameterSetName -ieq 'Encryption')
  {
    ...
    {
      Write-Error -Message 'Error creating TSV encryption key' -ErrorAction Stop
    }
  }
  ...
  foreach ($Computer in $ComputerName)
  {
    ...
    Write-Output -InputObject "Connecting to server '$($Computer)' to roll specified local admin passwords"
    ...
    if ($Result -ne $null)
    {
      ...
      {
        $Result |
        Select-Object -Property Username, Password, TargettedServerName, RealServerName |
        Export-Csv -Append -Path $TsvFileName -NoTypeInformation
      }
      else
      {
        #Filters out $null entries returned
        $Result = $Result | Select-Object -Property Username, Password, TargettedServerName, RealServerName
        ...
        foreach ($Record in $Result)
        {
          ...
          $Record |
          Select-Object -Property Username, EncryptedPassword, TargettedServerName, RealServerName |
          Export-Csv -Append -Path $TsvFileName -NoTypeInformation
        }
      }
    }
...
function ConvertTo-CleartextPassword
{
  ...
  Write-Output -InputObject ([System.Runtime.InteropServices.Marshal]::PtrToStringAuto([System.Runtime.InteropServices.Marshal]::SecureStringToCoTaskMemUnicode($SecureStringPassword)))
}
