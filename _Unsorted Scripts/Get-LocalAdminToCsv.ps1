...
function Get-LocalAdminToCsv {
    ...
    process {
       ...
       # New-Item -Path "$FolderName" -ItemType Directory -Force | Out-Null
        ...
        try {
            $Table  | Sort-Object Name | Select-Object Name, LocalAdmins | Export-Csv -path "$FolderName\Report.csv" -Append -NoTypeInformation
        }
        catch {
            Write-Warning $_
        }
...
Get-LocalAdminToCsv -Path c:\temp -ComputerName BUESMEWFP004
