...
#>
function Get-OrphanHomeFolder {
    [cmdletbinding(SupportsShouldProcess = $True)]
    param(
        [Parameter(Mandatory = $True)]
        $HomeFolderPath,
        [Object]$MoveFolderPath,
        [Object]$SearchBase,
        [string[]]$ExcludePath,
        [switch]$FolderSize,
        [switch]$MoveDisabled,
    ...
    if (!(Test-Path -LiteralPath $HomeFolderPath)) {
        Write-Warning -Message ('HomeFolderPath not found: {0}' -f $HomeFolderPath)
        exit
    }
    ...
    if ($MoveFolderPath) {
        if (!(Test-Path -LiteralPath $MoveFolderPath)) {
            Write-Warning -Message ('MoveFolderPath not found: {0}' -f $MoveFolderPath)
            exit
        }
    }

    # Main loop, for each folder found under home folder path AD is queried to find a matching samaccountname
    $ListOfFolders = Get-ChildItem -LiteralPath "$HomeFolderPath" -Force -Directory

    # Exclude folders if the ExcludePath parameter is given
    if ($ExcludePath) {
        $ExcludePath | ForEach-Object -Process {
            $CurrentExcludePath = $_
            if ($RegExExclude) {
                $ListOfFolders = $ListOfFolders | Where-Object -FilterScript {
                    $_.FullName -notmatch $CurrentExcludePath
                }
            }
            else {
                $ListOfFolders = $ListOfFolders | Where-Object -FilterScript {
                    $_.FullName -ne $CurrentExcludePath
                }
            }
        }
    }

    $ListOfFolders | ForEach-Object -Process {
        $CurrentPath = Split-Path -Path $_ -Leaf

        # Construct AD Searcher, add SearchRoot attribute if SearchBase parameter is specified
        $ADSearcher = New-Object -TypeName DirectoryServices.DirectorySearcher -Property @{
        ...
        if (!($ADResult)) {
            ...
            if ($MoveFolderPath) {
                ...
                if ($UseRobocopy) {
                    $null = C:\Windows\System32\Robocopy.exe $($HashProps.FullPath) $($HashProps.DestinationFullPath) /E /MOVE /R:2 /W:1 /XJD /XJF /MT:30
                }
                else {
            ...
            # Output the object # TODO [pscustomobject]
            $info = New-Object -TypeName PSCustomObject -Property $HashProps
            $info
    ...
    $HTML = New-HTMLHead -title 'Home Folder report' -style $style1
    $HTML += '<h3>Home folder clean up</h3>'
    $HTML += "<h4>Exclusions $($ExcludePath)</h4>"
    #$HTML += "<h4></h4>"
    #$HTML += "<h4>Script Started: $($StartTime)</h4>"
    $HTML += New-HTMLTable -InputObject $($result | Sort-Object -Property 'Error')
