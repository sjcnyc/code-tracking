...
function Copy-WithProgress 
{
  ...
  [RegEx]::Matches(($StagingContent -join "`n"), $RegexBytes) | ForEach-Object -Process {
    $BytesTotal = 0
  } {
    $BytesTotal += $_.Value
  }
  ...
  while (!$Robocopy.HasExited) 
  {
    ...
    $BytesCopied = [Regex]::Matches($LogContent, $RegexBytes) | ForEach-Object -Process {
      $BytesCopied += $_.Value
    } -End {
      $BytesCopied
    }
...
1..20 | ForEach-Object -Process {
  Set-Content -Path $TestSource\$_.txt -Value ('A'*(Get-Random -Minimum 10 -Maximum 2100))
}
...
21..40 | ForEach-Object -Process {
  Set-Content -Path $TestSource\$_.txt -Value ('A'*(Get-Random -Minimum 950 -Maximum 1400))
}
