function Send-PasswordExpiringNotification {
  ...
  try {
    $Accounts = Get-ADUser @getADUserSplat -Credential $Cred |Where-Object {$_.SamaccountName -like "svc_*"}

    foreach ($Account in $Accounts) {

      $User =
      Get-ADUser $Account -Properties "Name", "msDS-UserPasswordExpiryTimeComputed" -Credential $Cred |
      ...
      $Manager =
      Get-ADUser (Get-ADUser $Account  -properties * -Credential $Cred |Select-Object -ExpandProperty Manager) -properties * -Credential $Cred |Select-Object EmailAddress, Name
