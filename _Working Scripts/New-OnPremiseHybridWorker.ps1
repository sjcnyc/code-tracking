...
# admSConnea-azr@SonyMusicEntertainment.onmicrosoft.com


#Requires -RunAsAdministrator
function New-OnPremiseHybridWorker {

  Param (
    # Setup initial variables
    [Parameter(Mandatory = $true)]
    [ValidateSet("Global-RG", "IST-RG")]
    [String] $AAResourceGroupName,

    [Parameter(Mandatory = $false)]
    [String] $OMSResourceGroupName = "LogAnalytics-RG",

    [Parameter(Mandatory = $false)]
    [String] $SubscriptionID = "00c72111-4b19-4041-9490-ab6a640dee52",

    # OMS Workspace
    [Parameter(Mandatory = $false)]
    [String] $WorkspaceName = "LogAnalytics-SME",

    # Automation Account
    [Parameter(Mandatory = $true)]
    [ValidateSet("GlobalAccount", "ISTAccount")]
    [String] $AutomationAccountName,

    # Hyprid Group
    [Parameter(Mandatory = $true)]
    [ValidateSet("Tier-1", "Tier-2")]
  ...
  if ($OMSResouceGroupName) {
    ...
    # TODO: $OMSResourceGroupName = $AAResourceGroupName
  }
  ...
  try {
    ...
    # FIXME:
    # TODO: Write-Output "Creating new OMS Workspace named $WorkspaceName in region $OmsLocation..."
    # TODO: Create the new workspace for the given name, region, and resource group
    # TODO: $Workspace = New-AzureRmOperationalInsightsWorkspace -Location $OmsLocation -Name $WorkspaceName -Sku PerNode -ResourceGroupName $OMSResourceGroupName
    $Workspace = Get-AzureRmOperationalInsightsWorkspace -Name $WorkspaceName -ResourceGroupName $OMSResourceGroupName
...
$newOnPremiseHybridWorkerSplat_T1 = @{
  OMSResourceGroupName  = "LogAnalytics-RG"
  WorkspaceName         = "LogAnalytics-SME"
  AutomationAccountName = 'GlobalAccount'
  AAResourceGroupName   = 'Global-RG'
  HybridGroupName       = 'Tier-1'
  SubscriptionID        = "00c72111-4b19-4041-9490-ab6a640dee52"
}


$newOnPremiseHybridWorkerSplat_T2 = @{
  OMSResourceGroupName  = "LogAnalytics-RG"
  WorkspaceName         = "LogAnalytics-SME"
  AutomationAccountName = 'ISTAccount'
  AAResourceGroupName   = 'IST-RG'
  HybridGroupName       = 'Tier-2'
  SubscriptionID        = "00c72111-4b19-4041-9490-ab6a640dee52"
}
