    ...
    foreach ($printer in $printers) {
      ...
      if ($result.Status -eq 'Success') {
        ...
        if ($model) {
          ...
          switch -Regex ($model) {
            '^sharp' {
              try { $features = $($snmp.GetTree('.1.3.6.1.4.1.2385.1.1.3.2.1.3') | Where-Object { $_ -notlike '.*' }) -join ';' } catch { $features = $null }
              try { $name = $snmp.Get('.1.3.6.1.4.1.1536.1.3.5.4.2.0').toupper() } catch { $name = $null }
            }
            '^canon' {
              try {
                $name = $snmp.Gettree('.1.3.6.1.4.1.1602.1.3.3.1.1.2.1.1.10.134') |
          ...
          # ADDRESS
          try {
            $addr = ($snmp.Gettree('.1.3.6.1.2.1.4.20.1.1') |
              Where-Object { $_ -match '(?:[^\.]{1,3}\.){3}[^\.]{1,3}$' -and $_ -notmatch '127\.0\.0\.1' } |
              ForEach-Object { $ip = $_.split('.'); "$($ip[-4]).$($ip[-3]).$($ip[-2]).$($ip[-1])" }) -join ';' 
          }
          catch { $addr = $null }
