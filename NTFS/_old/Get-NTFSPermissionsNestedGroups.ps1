function Get-NTFSPermissionsNestedGroups {
  [CmdletBinding()]
  Param (
            ...
            {   $Name = $ACL.IdentityReference.Value.Split('\')[1]
                ...
                {   ForEach ($User in (Get-ADGroupMember $Name -Recursive | Select-Object -ExpandProperty Name))
                    {   $Result = New-Object PSObject -Property @{
                        ...
                        $Result | Select-Object Path,Group,User,FileSystemRights,AccessControlType,Inherited
                    }
                }
                Else
                {    $Result = New-Object PSObject -Property @{
                        Path = $Folder.Fullname
                        Group = ''
                        User = Get-ADUser $Name | Select-Object -ExpandProperty Name
                        FileSystemRights = $ACL.FileSystemRights
                        AccessControlType = $ACL.AccessControlType
                        Inherited = $ACL.IsInherited
                    }
