...
function TransformDHCPToArray ($filename,$Scope){ 
  ...
  for ($a = 0; $a -le $nbredeligne -1; $a++) { 
    if ($myinitfile[$a].contains('Subnet')){ 
      $personalarray = ''| select Subnet,NoOfAddressesinUse,NoOffreeAddresses, Comment 
...
function NetSHScopeDHCP($servername){ 
  ...
  $RemoteExec = (Get-WmiObject -List -ComputerName $servername | where {$_.name -eq 'win32_process'}).create($cmdline) 
