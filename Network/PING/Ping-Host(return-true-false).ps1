...
function Ping-Host  {
  ...
  try {
    ...
    if ($PingStatus.Status -eq 'Success'){$result = 1}
    else {$result = 0}
  }
  catch {$result = 0}
  ...
  $pingObj | Export-Csv -Path c:\temp\PingReport.csv -NoTypeInformation -Append
}
...
WBXL2054
ZRHCHEEULPT3905
"@ -split[environment]::NewLine | ForEach-Object -Process {Ping-Host -hostName $_}