...
function Get-IPInfo {
  ...
  PROCESS{
    ...
    If ([IPAddress]::TryParse($IPAddress,[ref]$TestIP)){
      Foreach ($IPAddress in $IPAddress) {GetIPInfo -IPAddr "$IPAddress" -mask "$SubnetMask"}
    } else {
      Write-Output -InputObject 'Please Enter a Valid IP'
    }
  } 
  END{}
}
function GetIPInfo {
  ...
  $SubnetMask = (([string]'255.') * $SimOctets)+ $Subnet
  }
...
New-Alias -Name IPCalc -Value Get-IPInfo

Export-ModuleMember -Function Get-IPInfo
Export-ModuleMember -Alias IPCalc