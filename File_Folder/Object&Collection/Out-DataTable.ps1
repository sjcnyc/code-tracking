...
function Get-Type
{
  ...
  if ( $types -contains $type ) {Write-Output -InputObject "$type"}
  else {Write-Output -InputObject 'System.String'}
} #Get-Type
  ...
  Process
  {
    foreach ($object in $InputObject)
    {
      ...
      {  
        ...
        if ($property.Gettype().IsArray) {$DR.Item($property.Name) = $property.value | ConvertTo-Xml -As String -NoTypeInformation -Depth 1}  
