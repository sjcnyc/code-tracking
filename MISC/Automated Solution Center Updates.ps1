...
function Get-LabTechConnection
{
	...
	{
		$txtToDecrypt = Get-ItemProperty HKLM:\Software\LabTech\Agent -Name MysqlPass | Select-Object -expand MySQLPass;
	}
	else
	{
		$txtToDecrypt = Get-ItemProperty HKLM:\Software\WOW6432Node\LabTech\Agent -Name MysqlPass | Select-Object -expand MySQLPass;
	}
...
attrib.exe -r $LTShareDir
icacls.exe $ltsharedir\* /T /Q /C /RESET
