...
function Convert-NewMedia {
  ...
  $plexIP = '192.168.1.3:32400'
  $plexToken = 'mgbVq2zp2b4EqVwc2AnZ'
  $ffmpeg = "$env:ProgramFiles\ffmpeg\bin\ffmpeg.exe"
  $handbrake = "$env:ProgramFiles\HandBrake\HandBrakeCLI.exe"
  ...
  ForEach ($file in $fileList)
  {
    ...
    ----------------------------------------------------------------------------------#>
    Clear-Host
    Write-Output -InputObject '------------------------------------------------------------------------------------' | Out-File $logPath -Append
    Write-Output -InputObject "$($time.Invoke()) Processing - $oldFile" | Out-File $logPath -Append
    Write-Output -InputObject "$($time.Invoke()) File $i of $fileCount - $progress%" | Out-File $logPath -Append
    ...
    -----------------------------------------------------------------------------------#>
    $ffCMD
    Write-Output -InputObject "$($time.Invoke()) ffmpeg completed" | Out-File $logPath -Append

    <#----------------------------------------------------------------------------------
        Refresh Plex libraries in Chrome
    -----------------------------------------------------------------------------------#>
    Invoke-WebRequest -Uri $plexURL 
    Write-Output -InputObject "$($time.Invoke()) Plex library refreshed" | Out-File $logPath -Append
    ...
    If ($fileNew.length -eq $fileOld.length) 
    {
      ...
      {
        Write-Output -InputObject "$($time.Invoke()) Same file size. $oldFile deleted." | Out-File $logPath -Append
      }
      Else 
      {
        Remove-Item $oldFile -Force
    ...
    Elseif ($fileNew.length -gt $fileOld.length) 
    {
      ...
      If ($confDelOld -eq $False)
      {
        Write-Output -InputObject "$($time.Invoke()) New file is $($diffGT)MB larger. $oldFile deleted." | Out-File $logPath -Append
      }
      Else 
      {
        Remove-Item $oldFile -Force
    ...
    Elseif ($fileNew.length -lt ($fileOld.length * .75) )
    {
      ...
      {
        Write-Output -InputObject "$($time.Invoke()) EXCEPTION: New file is over 25% smaller ($($diffErr)MB). $newFile deleted." | Out-File $logPath -Append
        Write-Output -InputObject "$($time.Invoke()) FAILOVER: Re-encoding $oldFile with Handbrake." | Out-File $logPath -Append
      }
      Else 
      {
        Remove-Item $newFile -Force
        ...
        $hbCMD = invoke-item $handbrake $hbArgs
			
        <#----------------------------------------------------------------------------------
            Begin HandbrakeCLI conversion (lossy) with supplied arguments
        -----------------------------------------------------------------------------------#>
        $hbCMD
        Write-Output -InputObject "$($time.Invoke()) Handbrake finished." | Out-File $logPath -Append
        Remove-Item $oldFile -Force
        If ($confDelOld -eq $False)
        {
          Write-Output -InputObject "$($time.Invoke()) $oldFile deleted." | Out-File $logPath -Append
        }
        Else 
        {
          Remove-Item $oldFile -Force
    ...
    Elseif ($fileNew.length -lt $fileOld.length)
    {
      ...
      If ($confDelOld -eq $False)
      {
        Write-Output -InputObject "$($time.Invoke()) New file is $($diffLT)MB smaller. $oldFile deleted." | Out-File $logPath -Append
      }
      Else 
      {
        Remove-Item $oldFile -Force
