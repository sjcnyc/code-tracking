...
function Call-ANUC_pff {
    ...
    $cboDomain_SelectedIndexChanged = {
        $cboPath.Items.Clear()
        Write-Verbose "Adding OUs to combo box"
        $XML.Options.Domains.Domain | ? {$_.Name -match $cboDomain.Text} | Select-Object -ExpandProperty Path | % {$cboPath.Items.Add($_)}
    ...
    $cboGroup_SelectedIndexChanged = { #20141120
        ...
        $Group = @($XML.Options.Groups.Group | Where-Objecthere-Object {$_.Name -match $cboGroup.Text}) #20141120
    ...
    $formMode_Click = {
        if ($formMode.Text -eq 'CSV Mode') {
            ...
            Get-Variable | Where-Objecthere-Object {$_.Name -match "txt"} | ForEach-Object {Try {$_.Value.Anchor = 'Top,Left'}catch {}}
