...
$Blobs = Get-AzStorageBlob -Container $SrcContainer -Context $SourceStorageContext |
  Where-Object { $_.ContentType -eq "application/xml" -and $_.name -like "00*" }
...
foreach ($Blob in $Blobs) {
  ...
  Get-AzStorageBlobContent @getAzStorageBlobContentSplat -WhatIf
  ...
  Start-AzStorageBlobCopy @startAzStorageBlobCopySplat -WhatIf

  Remove-AzStorageBlob -Context $SourceStorageContext -Container $SrcContainer -Blob $Blob.Name -WhatIf
}
...
"@
    $EmailParams = @{
      ...
      subject    = "Account not extended. Ticket# $($TicketXML.userAction.ticketnumber)"
