<#
.SYNOPSIS
   Scenario module for collecting AVD Smart Card data
...
function UEXAVD_GetRDGW {

    $RDSGWRAP = UEXAVD_GetRDRoleInfo Win32_TSGatewayResourceAuthorizationPolicy root\cimv2\TerminalServices
    if ($RDSGWRAP)
    { $RDSGWRAP | Out-File -Append ($RDSLogFolder + $LogFilePrefix + "rdgw_ResourceAuthorizationPolicy.txt") }
    else { UEXAVD_LogMessage $LogLevel.WarnLogFileOnly "[$LogPrefix] [WARNING] Failed to get Win32_TSGatewayResourceAuthorizationPolicy." }

    $RDSGWCAP = UEXAVD_GetRDRoleInfo Win32_TSGatewayConnectionAuthorizationPolicy root\cimv2\TerminalServices
    if ($RDSGWCAP)
    { $RDSGWCAP | Out-File -Append ($RDSLogFolder + $LogFilePrefix + "rdgw_ConnectionAuthorizationPolicy.txt") }
    else { UEXAVD_LogMessage $LogLevel.WarnLogFileOnly "[$LogPrefix] [WARNING] Failed to get Win32_TSGatewayConnectionAuthorizationPolicy." }

    $RDSGWS = UEXAVD_GetRDRoleInfo Win32_TSGatewayServerSettings root\cimv2\TerminalServices
    if ($RDSGWS)
    { $RDSGWS | Out-File -Append ($RDSLogFolder + $LogFilePrefix + "rdgw_ServerSettings.txt") }
    else { UEXAVD_LogMessage $LogLevel.WarnLogFileOnly "[$LogPrefix] [WARNING] Failed to get Win32_TSGatewayServerSettings." }
}

Function CollectUEX_AVDSCardLog {
    
    " " | Out-File -Append $OutputLogFile
    UEXAVD_LogMessage $LogLevel.Info ('Collecting Smart Card information')

    $Commands = @(
        "UEXAVD_GetEventLogs 'Microsoft-Windows-SmartCard-Audit/Authentication' 'SmartCard-Audit-Auth'"
        "UEXAVD_GetEventLogs 'Microsoft-Windows-SmartCard-DeviceEnum/Operational' 'SmartCard-DeviceEnum-Operational'"
        "UEXAVD_GetEventLogs 'Microsoft-Windows-SmartCard-TPM-VCard-Module/Admin' 'SmartCard-TPM-Admin'"
        "UEXAVD_GetEventLogs 'Microsoft-Windows-SmartCard-TPM-VCard-Module/Operational' 'SmartCard-TPM-Operational'"
        "certutil -scinfo -silent 2>&1 | Out-File -Append '" + $SysInfoLogFolder + $LogFilePrefix + "SmartCardInfo.txt'"
    )
    ...
    if ($ver -like "*Windows Server*") {
        ...
        $Commands = @(
            "UEXAVD_GetEventLogs 'Microsoft-Windows-Kerberos-KDCProxy/Operational' 'Kerberos-KDCProxy-Operational'"
        )
        UEXAVD_RunCommands $LogPrefix $Commands -ThrowException:$False -ShowMessage:$True -ShowError:$True
        ...
        if ($isRDGW -eq "Installed") {
            " " | Out-File -Append $OutputLogFile
            UEXAVD_LogMessage $LogLevel.Info ('Collecting Remote Desktop Gateway information')
            
            UEXAVD_CreateLogFolder $RDSLogFolder
            UEXAVD_GetRDGW
        } else {
            UEXAVD_LogMessage $LogLevel.WarnLogFileOnly "[$LogPrefix] RD Gateway role not found. Skipping collection of RD Gateway information"
        }
    } else {
            UEXAVD_LogMessage $LogLevel.WarnLogFileOnly "[$LogPrefix] Windows Server OS not found. Skipping collection of KDCProxy and RD Gateway information"
    }
}

Export-ModuleMember -Function CollectUEX_AVDSCardLog
# SIG # Begin signature block
# MIIntwYJKoZIhvcNAQcCoIInqDCCJ6QCAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC