    ...
    End {
        ...
        $Task | ForEach-Object {
            ...
            $Object = [pscustomobject]@{
                Computername = $_.Computernamepowershell
                IPAddress    = $IPAddress
                Result       = $Result
            }
...
foreach ($comp in $comps) {
    ...
    $pingtest = Test-ConnectionAsync -Computername "$($comp).me.sonymusic.com" -Timeout 100
        ...
        $PSObj = [pscustomobject]@{
            'ComputerName' = "$($comp).me.sonymusic.com"
...
$PSArray = $PSArray | where {$_.result -ne "Success"}
...
$importance = "Failure", "TimeOut", "Success"
...
$summaryTable = [PSCustomObject] @{
   ...
   # Add-HTMLTableColor -Column "Success" -AttrValue "background-color:#c6ffb3;" @params1 |
    ...
    $HTML += $summaryTable
       ...
       # Add-HTMLTableColor -Argument 'Success' -Column "Result" -AttrValue "background-color:#c6ffb3;" @params
