...
OU=Service,OU=Users,OU=GBL,OU=USA,OU=NA,OU=ADM,OU=Tier-0,DC=me,DC=sonymusic,DC=com
...
foreach ($user in $users) {
  ...
  $tier            = if ($user.DistinguishedName -like "*Tier-2*") { "Tier-2" } else { "Tier-1" }
  ...
  if (($daystoexpire -ge "0") -and ($daystoexpire -lt $expireindays)) {
    ...
    if ($PSArrayList.Count -ne '0') {
      ...
      $EmailParams = @{
        ...
        Subject    = "Your $($tier) service account password expires in $($daystoexpire) days"
